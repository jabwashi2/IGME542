...
void Game::LoadMaterials()// textures and materials
{
	...
	// emitter SRVS
	Microsoft::WRL::ComPtr<ID3D11ShaderResourceView> sparkSRV;
	Microsoft::WRL::ComPtr<ID3D11ShaderResourceView> flameSRV;
	Microsoft::WRL::ComPtr<ID3D11ShaderResourceView> smokeSRV;

	// loading emitter textures
	CreateWICTextureFromFile(device.Get(), context.Get(), FixPath(L"../../Assets/Particles/PNG (Transparent)/spark_06.png").c_str(), 0, sparkSRV.GetAddressOf());
	CreateWICTextureFromFile(device.Get(), context.Get(), FixPath(L"../../Assets/Particles/PNG (Transparent)/flame_02.png").c_str(), 0, flameSRV.GetAddressOf());
	...
	spark6->AddTextureSRV("ParticleTexture", sparkSRV);

	flame2->AddSampler("BasicSampler", sampler);
	flame2->AddTextureSRV("ParticleTexture", flameSRV);

	smoke4->AddSampler("BasicSampler", sampler);
	smoke4->AddTextureSRV("ParticleTexture", smokeSRV);
}
...
void Game::Update(float deltaTime, float totalTime)
{
	...
	// emitters
	sparkEmitter->Update(deltaTime, totalTime);
	flameEmitter->Update(deltaTime, totalTime);
	smokeEmitter->Update(deltaTime, totalTime);
