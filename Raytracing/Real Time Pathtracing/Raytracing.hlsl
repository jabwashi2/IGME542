...
{
    // (ty for this Chris) If we've reached the max recursion, we haven't hit a light source
    if (payload.recursionDepth == 10)
    {
        // black if we don't hit anything
        payload.color = float3(0, 0, 0);
        return;
    }
    
    // if we dooooo hit something, tint by that thing's surface color
